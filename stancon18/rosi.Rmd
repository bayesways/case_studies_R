...
In our case, we want to save the data and the posterior samples for an extra reason. 
Later on, we will need to demonstrate how to fit the model on the real datasets. For the purposes of this notebook, we will re-use this data as if it was real. The choice for the name "control" will become clearer later. We can save the data and the samples as follows
```{r eval=TRUE}
